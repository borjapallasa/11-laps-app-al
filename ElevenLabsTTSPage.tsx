 ...
 * NOTE: This build calls a local **server proxy** so your API key is never exposed to the browser.
 */

const ELEVEN_VOICES_URL = "/api/voices";
const ELEVEN_HISTORY_URL = "/api/history";

function useFetchVoices() {
...
function useHistoryLazy(active: boolean) {
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => {
    if (!active || items.length || loading) return;
    (async () => {
      setLoading(true);
      setError("");
      try {
...
export default function ElevenLabsTTSPage() {
  const { voices, loading: loadingVoices, error: voicesError } = useFetchVoices();
  ...
  const { items: historyItems, loading: loadingHistory, error: historyError } = useHistoryLazy(isHistoryActive);
  ...
  const playHistoryAudio = async (it: any) => {
    try {
      setHasAudio(true);
      const endpoint = `/api/history/${it.history_item_id}/audio`;
  ...
  return (
    <div className={"min-h-screen w-full bg-white text-neutral-900 " + (hasAudio ? "pb-28" : "")}> 
      <VoicesStrip
        voices={voices}
        error={voicesError}
      ...
      <main className="mx-auto hidden max-w-7xl border-t border-neutral-200 px-6 pb-8 overflow-hidden min-h-0 h-[calc(100vh-120px)] md:block">
        <div className="md:flex md:items-stretch gap-8 min-h-0 h-full">
          ...
          <aside className="pt-8 md:px-4 md:w-[380px] flex flex-col h-full min-h-0">
            <div className="sticky top-6 z-10 bg-white">
              <Tabs value={tab} onChange={setTab} items={["Configuration", "History"]} />
            </div>
            <div className={"mt-2 flex-1 min-h-0 overflow-hidden " + (hasAudio ? "pb-28" : "pb-4") }>
              {tab === "config" ? (
                <ConfigPanel
                  voices={voices}
                  selectedVoiceId={selectedVoiceId}
...
function Editor({ text, onChange, hasAudio = false }: { text: string; onChange: (v: string) => void; hasAudio?: boolean }) {
  ...
  return (
    <section className="pt-8 pr-4 md:pr-8">
      ...
      <div className={`md:hidden fixed inset-x-0 ${hasAudio ? "bottom-32" : "bottom-[calc(env(safe-area-inset-bottom)+12px)]"} z-30 px-4`}>
        <Btn className="w-full shadow-lg">Generate audio</Btn>
        <Btn variant="ghost" className="mt-2 w-full">Export to Project</Btn>
      </div>
    </section>
  );
}
...
function ConfigPanel(props: any) {
  const { voices, selectedVoiceId, onVoice, model, onModel, speed, onSpeed, stability, onStability, similarity, onSimilarity, style, onStyle, speakerBoost, onSpeakerBoost } = props;
  return (
    <div className="space-y-6">
      ...
      <div className="-mt-[10px]"><Btn className="w-full">Generate audio</Btn></div>
      <div className="mt-2"><Btn variant="ghost" className="w-full">Export to Project</Btn></div>
    </div>
  );
}
...
function HistoryPanel({ query, onQuery, items, loading, error, onSelect }: any) {
  return (
    <div className="space-y-3 h-full min-h-0">
      ...
      <div className="min-h-0 max-h-[480px] overflow-auto pr-1">
...
function Btn({ children, variant = "solid", className = "" }: any) {
